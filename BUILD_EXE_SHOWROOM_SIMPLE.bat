@echo off
title Build EXE File - Showroom
color 0B

echo ========================================
echo    BUILD EXE FILE - SHOWROOM
echo ========================================
echo.
...
if exist "KiemKhoShowroomApp.spec" (
    echo [INFO] Deleting old spec file...
    del /q KiemKhoShowroomApp.spec
)
...
echo # File nay duoc tao tu dong khi build exe > dist_path_config.py
echo # Chua duong dan thu muc dist goc >> dist_path_config.py
echo DIST_PATH = None  # Se duoc cap nhat sau khi build >> dist_path_config.py

python -m PyInstaller --onefile --windowed --name "KiemKhoShowroomApp" ^
...
if exist "DuLieuDauVaoShowroom.xlsx" (
    if not exist "dist" mkdir dist
    copy "DuLieuDauVaoShowroom.xlsx" "dist\" >nul
...
if exist "dist" (
    ...
    echo [INFO] dist folder now contains only: KiemKhoShowroomApp.exe and DuLieuDauVaoShowroom.xlsx
)
...
echo ========================================
echo.
echo Executable file created at: dist\KiemKhoShowroomApp.exe
echo.
